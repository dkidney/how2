...
`-r` recursive `-v` invert - i.e. show non-matching lines  
...
```
  ...
  - for more than one fileIf you give less the names of several files,
    you can type
...
`Ctrl-a n` go to the next window `Ctrl-a p` go to the previous window
...
#### pipe
  ...
  - use the output of the command on the left as the input to the
...
### Wildcards
  ...
  - `{...}` matches any of the comma-separated patterns inside the
    brackets, e.g. `{*.txt, *.csv}`
